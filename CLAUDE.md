...
- **Limite costo mensile**: per utente, letto da `user_limits.limite_mensile` con fallback $5/mese (costante `COSTO_MENSILE_MAX` in `/api/chat/route.ts`). Calcolato in parallelo al rate limiting sommando `costo_stimato` dei messaggi dal primo del mese. Restituisce 429 con messaggio che invita a contattare il supporto. Per alzare il limite di un utente: `INSERT INTO user_limits ... ON CONFLICT DO UPDATE` via SQL Editor (la tabella non ha policy di scrittura — l'utente non può modificarsi il limite, vedi `supabase/rls/user_limits_migration.sql`).
