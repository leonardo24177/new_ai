...
- **Ricerca web — fetch contenuto pagine**: dopo i risultati Serper, le prime 3 URL vengono visitate da `r.jina.ai` (timeout 8s) per estrarne il testo completo (max 4.000 char/pagina). Il testo viene passato a Claude come `raw_content`. Se la pagina richiede login o jina fallisce, si usa lo snippet Serper come fallback. I siti paywalled (es. Simpliciter, DeJure) non sono accessibili — l'utente deve allegare il PDF.
- **Ricerca web — REGOLA_FONTI**: quando c'è contesto di ricerca web, il chat route aggiunge un'istruzione al system prompt che sblocca la `REGOLA_FONTI` per quei risultati — Claude li tratta come fonti verificate e non antepone l'avviso ⚠️.
