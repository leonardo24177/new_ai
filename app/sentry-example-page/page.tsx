...
export default function SentryTestPage() {
  function sendError() {
    // Assicura l'inizializzazione anche se instrumentation.client.ts non è stato caricato
    const client = Sentry.getClient()
    if (!client) {
      Sentry.init({ dsn: SENTRY_DSN, environment: 'development' })
    }
    Sentry.captureException(new Error('Test error da assistente-ai.it'))
    alert('Errore inviato! Controlla su sentry.io → Issues')
  }
