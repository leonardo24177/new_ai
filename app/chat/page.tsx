...
export default function ChatPage() {
  ...
  // Fix per iOS: previeni zoom su focus input
  useEffect(() => {
  ...
  async function sendMessage() {
    ...
    if (textareaRef.current) {
      ...
      textareaRef.current.blur() // chiude tastiera su mobile dopo invio
    }
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    // Su mobile non inviamo con Enter (userebbe la tastiera virtuale)
  ...
  return (
    // h-dvh usa l'altezza dinamica del viewport (tiene conto della barra browser mobile)
    <div className="flex h-[100dvh] bg-white overflow-hidden">

      {/* Dialog scelta file */}
      {showFileDialog && pendingFile && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 pb-safe px-4">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xl p-5 w-full max-w-sm mb-4">
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full px-4 py-3.5 rounded-xl border border-gray-200 text-left active:bg-gray-50 transition-colors"
              >
            ...
            <button
              ...
              className="w-full mt-3 text-sm text-gray-400 py-2 active:text-gray-600"
            >
              Annulla
            </button>
          </div>
      ...
      {/* Sidebar Drawer — full height, 80% larghezza su mobile */}
      <div className={`fixed inset-y-0 left-0 z-30 w-4/5 max-w-xs bg-gray-50 border-r border-gray-200 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-gray-200 flex items-center justify-between pt-safe">
          <h2 className="text-sm font-semibold text-gray-900">Conversazioni</h2>
          <button
            onClick={() => setSidebarOpen(false)}
            className="w-8 h-8 flex items-center justify-center text-gray-400 active:text-gray-600 rounded-lg"
          >
            ✕
          </button>
        </div>
        <div className="p-3 border-b border-gray-200">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-3 rounded-xl bg-gray-900 text-white text-sm font-medium active:bg-gray-800 transition-colors"
          >
            <span>+</span>
          ...
          {loadingHistory ? (
            ...
            conversations.map(conv => (
              <div
                ...
                className={`group flex items-center justify-between px-3 py-3 rounded-xl cursor-pointer transition-colors ${conv.id === conversationId ? 'bg-gray-200' : 'active:bg-gray-100'}`}
              >
                ...
                <button
                  ...
                  className="text-gray-300 active:text-red-500 ml-2 text-base p-1"
                >
                  🗑
                </button>
              </div>
        ...
        {/* Footer sidebar */}
        <div className="p-4 border-t border-gray-200 space-y-1 pb-safe">
          <button
            onClick={() => router.push('/profile')}
            className="w-full text-sm text-gray-600 active:text-gray-900 transition-colors text-left py-2 flex items-center gap-3"
          >
            <span>👤</span> Profilo
          </button>
          {isAdmin && (
          ...
          <button
            ...
            className="w-full text-sm text-red-500 active:text-red-700 transition-colors text-left py-2 flex items-center gap-3"
          >
            <span>→</span> Esci
          </button>
        </div>
      ...
      {/* Main chat — usa flex column con altezza dinamica */}
      <div className="flex flex-col flex-1 min-w-0 min-h-0">

        {/* Header */}
        <div className="border-b border-gray-100 px-4 py-3 flex items-center justify-between flex-shrink-0 pt-safe bg-white">
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setSidebarOpen(true); loadConversations() }}
              className="w-9 h-9 flex items-center justify-center text-gray-400 active:text-gray-600 transition-colors rounded-xl"
            >
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{nomeAssistente}</p>
              {nomeUtente && <p className="text-xs text-gray-400 truncate">Ciao, {nomeUtente}</p>}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={newConversation}
              className="text-xs text-gray-400 active:text-gray-600 transition-colors px-2 py-1"
            >
              + Nuova
            </button>
          </div>
        ...
        {/* Messaggi — flex-1 con overflow-y-auto */}
          ...
          <div className="px-4 pb-2 flex-shrink-0">
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                className="text-xs text-gray-400 active:text-gray-600 flex items-center gap-1 py-1"
              >
        ...
        {/* Input area — sempre visibile, non compresso dalla tastiera */}
        <div className="border-t border-gray-100 px-3 py-3 flex-shrink-0 pb-safe bg-white">
          {/* Selector ambito */}
          {ambitiDisponibili.length > 1 && (
            <div className="flex items-center gap-2 mb-2 relative">
              ...
              {showAmbitoMenu && (
                <div className="absolute bottom-9 left-0 bg-white border border-gray-200 rounded-xl shadow-lg p-1 z-10 min-w-[140px]">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm transition-colors ${
                          ambitoAttivo === a ? 'bg-gray-900 text-white' : 'text-gray-700 active:bg-gray-50'
                        }`}
                      >
                        <span>{cfg?.emoji}</span>
                        <span>{cfg?.label}</span>
          ...
          <div className="flex items-end gap-2 bg-gray-50 rounded-2xl px-3 py-2.5">
            <button
            ...
            <textarea
              ...
              placeholder={`Scrivi un messaggio${ambitoAttivo ? ` [${ambitoAttivo}]` : ''}...`}
              rows={1}
              className="flex-1 bg-transparent resize-none focus:outline-none text-sm text-gray-900 placeholder:text-gray-400 max-h-[120px] leading-relaxed"
            />
            <button
              onClick={sendMessage}
              disabled={loading || !input.trim()}
              className="w-8 h-8 flex items-center justify-center bg-gray-900 text-white rounded-xl disabled:opacity-30 active:bg-gray-700 transition-colors flex-shrink-0"
            >
